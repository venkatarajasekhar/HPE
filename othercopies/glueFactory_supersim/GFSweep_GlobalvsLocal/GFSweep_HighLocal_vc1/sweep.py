...
def main(args):
  ...
  for load in loads:
    ...
    settings_json = os.path.join(args.ssenv, 'poem_supersim', 'json',
                                 'hierarchyhyperx_iq_GlueFactory2.json')
    ...
    sim_cmd = ('{0} {1} '
               ...
               'network.router.input_queue_depth=uint={6} '
               'network.num_vcs=uint={7} ').format(
                 supersim_bin, settings_json, load, messages_mpf, rates_csv,
                 channels_csv, args.queue, args.vc)
  ...
  loadlat_cmd = ('ssllp --row Packet --ymax 500000 --title "QueueSize={0} VC={5}" '
                 '{1} {2} {3} {4}').format(args.queue, loadlat_file, sweepStart,
                                           sweepStop+1, sweepStep, args.vc)
  agg_files = []
  for load in loads:
    runId = '{0:02d}_{1}'.format(args.queue, load)
    aggregate_csv = os.path.join(args.rundir, 'aggregate_{0}.csv'.format(runId))
...
if __name__ == '__main__':
  ...
  ap.add_argument('-vc', '--vc', type=int, default=1,
                  help='number of vcs')
