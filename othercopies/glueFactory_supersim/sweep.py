...
def main(args):
  ...
  parse_tasks = []
  for load in loads:
    runId = '{0}_{1}_{2}'.format(args.traffic, args.routing, load)
    ...
    sim_cmd = ('{0} {1} '
               ...
               'network.routing.algorithm=string={6} '
               'application.blast_terminal.traffic_pattern.type=string={7} ').format(
                 supersim_bin, settings_json, load, messages_mpf, rates_csv,
                 channels_csv, args.routing, args.traffic)
  ...
  loadlat_file = os.path.join(args.rundir,
                              'load_latency_{0}_{1}.png'.format(args.traffic, args.routing))
  loadlat_cmd = ('ssllp --row Packet --ymax 1000 --title "Traffic={0} Routing={1}" '
                 '{2} {3} {4} {5}').format(args.traffic, args.routing, loadlat_file, sweepStart,
  ...
  loadlat_task = taskrun.ProcessTask(tm, 'loadlat_{0}_{1}'.format(args.traffic, args.routing),
                                     loadlat_cmd)
...
if __name__ == '__main__':
  ...
  ap.add_argument('-t', '--traffic', type=str, default='tornado',
                  help='traffic pattern')
  ap.add_argument('-r', '--routing', type=str, default='valiants',
