 ...
 */
#include "network/hierarchicalhyperx/UniversalAdaptiveRoutingAlgorithm.h"
...
UniversalAdaptiveRoutingAlgorithm::UniversalAdaptiveRoutingAlgorithm(
...
void UniversalAdaptiveRoutingAlgorithm::processRequest(
...
std::unordered_set<u32> UniversalAdaptiveRoutingAlgorithm::routing(Flit* _flit,
  const std::vector<u32>* destinationAddress) {
  // ex: [1,...,m,1,...,n]
  ...
  if (atGlobalDst == false) {
    ...
    if (std::equal(localDst->begin(), localDst->end(),
          routerAddress.begin())) {
      // determine if go valiant or not
...
u32 UniversalAdaptiveRoutingAlgorithm::getPortBase() {
  u32 localDimensions = localDimWidths_.size();
...
void UniversalAdaptiveRoutingAlgorithm::globalPortToLocalAddress(u32 globalPort,
...
u32 UniversalAdaptiveRoutingAlgorithm::findHighestPort(
  std::unordered_map< u32, f64 > portAvailability) {
...
void UniversalAdaptiveRoutingAlgorithm::findPortAvailability(
...
void UniversalAdaptiveRoutingAlgorithm::setLocalDst(
...
void UniversalAdaptiveRoutingAlgorithm::ifAtLocalDst(Flit* _flit,
  std::unordered_set<u32>* outputPorts,
