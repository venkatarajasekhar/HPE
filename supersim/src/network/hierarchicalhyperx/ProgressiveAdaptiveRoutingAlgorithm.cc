...
void ProgressiveAdaptiveRoutingAlgorithm::processRequest(
  ...
  if (_flit->getPacket()->getHopCount() == 1) {
    _flit->getPacket()->setValiantMode(false);
  }

  std::unordered_set<u32> outputPorts;
  // routing depends on mode
  if (_flit->getPacket()->getValiantMode() == false) {
    outputPorts = routing(_flit, destinationAddress);
  } else {
    // use Valiant routing
  ...
  if (packet->getGlobalHopCount() == 0 && globalDim != globalDimensions) {
    ...
    if (std::equal(localDst->begin(), localDst->end(),
      ...
      if (outputPorts.size() == 0) {
        ...
        std::unordered_set<u32> outputPorts = ValiantRoutingAlgorithm::routing(
                                              _flit, destinationAddress);
        assert(outputPorts.size() >= 1);
      }
    } else {
